...
当前 v0.5.1 开发线已经包含可运行的本地加速内核，支持 ProxyOnly、PAC、System Proxy、Windows Hosts 反代、Steam 域名匹配、YAML 配置、可配置 DNS 解析与缓存、IPv4 / IPv6 策略、Direct / HTTP / SOCKS5 上游出口、本地 rollback 状态、前台 CLI 生命周期、本地状态文件，以及带 token 的 loopback 控制接口。Hosts + Direct 模式默认会使用内置 DoH 解析真实 Steam IP，避免 hosts 自绕回；HTTP / SOCKS5 upstream 是可选增强，不是默认加速前提。`v0.5.1-dev` 会在出站失败时显示 DoH / TCP / TLS 失败摘要，便于定位真实网络问题。
...
Resolver、upstream、PAC、System Proxy 和 Hosts 选项通过 YAML 配置。通用默认仍为 `resolver.mode: system` 与 `upstream.type: direct`；但 `start --mode hosts` 在 Direct 出口下会自动使用内置 DoH，`status` 会显示 `resolver: doh`。
