...
当前 v0.6.0 已经包含可运行的本地加速内核，支持 ProxyOnly、PAC、System Proxy、Windows Hosts 反代、Steam 域名匹配、YAML 配置、可配置 DNS 解析与缓存、IPv4 / IPv6 策略、Direct / HTTP / SOCKS5 上游出口、本地 rollback 状态、前台 CLI 生命周期、本地状态文件，以及带 token 的 loopback 控制接口。Hosts + Direct 模式默认会使用内置 DoH 解析真实 Steam IP，避免 hosts 自绕回；并已加入默认 Steam 出站 profile，让 community 域名优先走 `steamcommunity-a.akamaihd.net`，store / checkout / help / login / media 域名优先走 `cdn-a.akamaihd.net`，并覆盖 `community.steamstatic.com` 与 `steamcdn-a.akamaihd.net` 这类常见静态资源 / CDN 域名。HTTP Host 保留原始 Steam 域名，TLS SNI 按 profile 使用可达 CDN 域名。启动探测会在 `start` 和 `status` 中暴露 DoH / TCP / TLS / HTTP smoke 状态。HTTP / SOCKS5 upstream 是可选增强，不是默认加速前提。
...
- 本地 Root CA 生成、Windows 当前用户证书库安装/卸载。
- Hosts 模式下的本地 HTTP / HTTPS Reverse Proxy 与动态站点证书。
- Windows Hosts 一键流程：Root CA 自动安装、hosts preflight、rollback 和系统修改状态输出。
...
Windows Hosts 模式默认会在启动流程内检查并安装本地 Root CA。hosts 写入仍要求当前进程或外层 wrapper 具备对应权限：
