...
当前 v0.6.4-dev 已经包含可运行的本地加速内核，支持 ProxyOnly、PAC、System Proxy、Windows Hosts 反代、Steam 域名匹配、YAML 配置、可配置 DNS 解析与缓存、IPv4 / IPv6 策略、Direct / HTTP / SOCKS5 上游出口、本地 rollback 状态、前台 CLI 生命周期、本地状态文件，以及带 token 的运行时控制接口。Hosts + Direct 模式默认会使用内置 DoH 解析真实 Steam IP，避免 hosts 自绕回；并已加入默认 Steam 出站 profile，让 community 域名优先走 `steamcommunity-a.akamaihd.net`，store / checkout / help / login / media 域名优先走 `cdn-a.akamaihd.net`，并覆盖 `community.steamstatic.com` 与 `steamcdn-a.akamaihd.net` 这类常见静态资源 / CDN 域名。HTTP Host 保留原始 Steam 域名，TLS SNI 按 profile 使用可达 CDN 域名。启动探测会在 `start` 和 `status` 中暴露 DoH / TCP / TLS / HTTP smoke 状态。Windows 系统写入现在走接近 Steam++ 的 AppHost Service 路线：先用管理员授权安装一次 `SiteBoostCoreAppHost`，之后普通 PowerShell 可通过本机 named pipe `\\.\pipe\SiteBoostCoreAppHost` 请求受限 Root CA、hosts 和恢复动作；HTTP / SOCKS5 upstream 是可选增强，不是默认加速前提。
...
12. `v0.7.0`：通用加速核心重构与命名迁移准备。
13. `v1.0.0`：稳定 API 与集成发布。
