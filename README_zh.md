...
当前 v0.3.0 开发线已经包含可运行的本地加速内核，支持 ProxyOnly、PAC、System Proxy、Steam 域名匹配、YAML 配置、可配置 DNS 解析与缓存、IPv4 / IPv6 策略、Direct / HTTP / SOCKS5 上游出口、本地 rollback 状态、前台 CLI 生命周期、本地状态文件，以及带 token 的 loopback 控制接口。
...
Resolver、upstream、PAC 和 System Proxy 选项通过 YAML 配置。默认仍为 `resolver.mode: system` 与 `upstream.type: direct`。
