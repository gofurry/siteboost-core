...
当前 v0.2.0 开发线已经包含可运行的 ProxyOnly 内核，支持本地 HTTP Proxy、HTTPS CONNECT 隧道、Steam 域名匹配、YAML 配置、可配置 DNS 解析与缓存、IPv4 / IPv6 策略、Direct / HTTP / SOCKS5 上游出口、前台 CLI 生命周期、本地状态文件，以及带 token 的 loopback 控制接口。
...
Resolver 和 upstream 选项通过 YAML 配置。默认仍为 `resolver.mode: system` 与 `upstream.type: direct`。
