...
> 状态日期：2026-06-18
> 主维护语言：中文  
> 当前阶段：v0.4.0 Hosts + HTTPS Reverse Proxy 已实现，等待后续打磨与发布
...
- [x] 实现 Config 默认值、YAML 加载与校验。
- [x] 实现 Steam 默认域名规则。
- [x] 实现 `rules.Matcher`，支持精确、`*.` 通配符、端口剥离、大小写处理与 IDNA。
- [x] 实现 HTTP Proxy 基础框架。
- [x] 实现 HTTPS CONNECT 隧道。
- [x] 实现 Direct upstream。
- [x] 实现基础结构化日志，避免记录 Cookie、Authorization 和完整 query。
...
- [x] 实现 `Resolver` 接口。
- [x] 实现 system / udp / tcp / doh resolver。
- [x] 实现 DNS 缓存、超时和备用 DNS fallback。
- [x] 实现 IPv4 / IPv6 偏好与禁用策略。
- [x] 实现 Direct / HTTP Proxy / SOCKS5 upstream。
- [x] 支持代理认证配置并避免日志泄露密码。
- [x] 在 proxy Dial 链路中接入 resolver + upstream。
...
- [x] 实现 PAC 生成器，规则来源统一来自 `rules` 模块。
- [x] 实现 PAC Server，默认监听 `127.0.0.1`。
- [x] 实现 `start --mode pac`。
- [x] 实现 Windows 系统 PAC 写入与恢复。
- [x] 实现 macOS 系统 PAC 写入与恢复。
- [x] 实现 Windows 系统 HTTP / HTTPS 代理写入与恢复。
- [x] 实现 macOS 系统 HTTP / HTTPS 代理写入与恢复。
- [x] 实现 rollback 状态文件。
- [x] 实现 `restore` 命令。
...
- [x] 实现 hosts patcher，只管理项目标记区块。
- [x] 实现 hosts 备份、失败回滚与 `restore` 恢复。
- [x] 实现 Root CA 生成。
- [x] 实现 Root CA 安装与卸载。
- [x] 实现动态站点证书签发与缓存。
- [x] 实现本地 HTTP Server。
...
- `restore` 可移除 hosts 项目标记区块；Root CA 由 `cert uninstall` 显式卸载。

#### 说明

- v0.4.0 为 Windows-first，macOS / Linux Hosts 与证书安装明确返回不支持。
- hosts 文件不能表达 `*.` 通配符，本版本只写入 exact 规则与用户显式配置域名。
- Root CA 由 `cert install` / `cert uninstall` 显式管理，`restore` 不自动卸载证书。
...
- 继续用补丁版本打磨 v0.4.0 的 Hosts / HTTPS Reverse Proxy 与 restore 路径。
- 补充真实平台 smoke 记录、hosts 权限和证书边界测试。
- 保持 CI 和文档同步更新。

中期：

- 评估 macOS / Linux Hosts 与证书安装支持。
- 建立更完整的跨平台恢复策略。
