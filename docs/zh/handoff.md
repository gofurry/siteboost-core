...
- `version.go` 已显示 `v0.7.2-dev`。
- 主干代码已经进入 `v0.7.2-dev` 阶段，包含 AppHost Service 自动启动、named pipe IPC、provider registry、Steam stable provider、GitHub experimental skeleton provider、DNSIntercept manual 本地 DNS server 和 Windows system DNS 显式接管 / rollback / restore。
...
- Hosts 模式只能覆盖 exact 域名；DNSIntercept manual 可覆盖 wildcard，但只在显式 `mode: dns` 下启动，不会自动接管系统 DNS。DNSIntercept system 可显式接管 Windows 指定网卡 DNS，但仍需补真实 Windows smoke。
...
1. 跑完 `v0.7.2-dev` 自动化验证：`git diff --check`、`go test ./...`、`go vet ./...`、race 子集、build、version。
...
3. 补做 DNSIntercept system Windows smoke：先记录 `Get-DnsClientServerAddress -AddressFamily IPv4`，使用 `strategy: system`、`listen_addr: 127.0.0.1:53` 和显式 `interfaces` 启动，确认 `system_dns=true`，再 `stop` / `restore` 并确认 DNS 恢复。
4. 补做默认 Steam Hosts + DoH + AppHost 真实 Windows smoke，并确认 `status` 同时显示 `provider: id=steam ...` 和兼容 `rule_set: steam-web@...`。
5. 补做 `providers.enabled: [steam, github]` skeleton smoke：确认 GitHub 显示 `experimental`，但不要求 GitHub live 可达，也不写成真实加速能力。
6. 补做单独重启 smoke：`reboot -> apphost status health=ok -> normal-user start --mode hosts -> stop/restore`，并把输出补进 smoke 文档。
7. 进入 v0.7.3 Page Enhance 透明 pipeline。
8. 整理未来独立 Go library 的 API 草案和迁移清单：`Config`、`Engine`、`Provider`、`Mode`、`Status`、`Start`、`Stop`、`Restore`。
9. 继续设计 AppHost IPC 加固：优先评估用户会话绑定、审计日志和按需启动。
10. 进入 release engineering：CI matrix、rollback schema version、installer、服务升级 / 卸载、签名规划。
11. 在验证充分后新建正式 Go library 仓库，从本仓库迁移已验证的 resolver、reverse、certstore、privilege、provider、dnsintercept、systemdns 和 diagnostics 能力。
