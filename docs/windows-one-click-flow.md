...
The default `machine` scope writes to `LocalMachine\Root`, which is the low-friction path for administrator-run Hosts mode and avoids the first-run confirmation commonly seen with `CurrentUser\Root`. Use `cert.store_scope: user` only when a current-user trust store is required.
