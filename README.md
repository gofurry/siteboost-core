...
The current v0.5.1 development line includes a runnable local acceleration core. It supports ProxyOnly, PAC, System Proxy, and Windows-first Hosts reverse proxy modes, Steam domain matching, YAML configuration, configurable DNS resolution with cache and IP policy, direct/HTTP/SOCKS5 upstream dialing, local rollback state, a foreground CLI lifecycle, a local state file, and a token-protected loopback control interface. Hosts + Direct mode uses built-in DoH defaults for real Steam IP lookup to avoid local hosts loopback; HTTP/SOCKS5 upstreams are optional enhancements, not the default acceleration prerequisite. `v0.5.1-dev` also reports DoH / TCP / TLS outbound failure summaries so real network failures can be diagnosed.
...
Resolver, upstream, PAC, system proxy, and Hosts options are configured through YAML. The general defaults remain `resolver.mode: system` and `upstream.type: direct`; `start --mode hosts` automatically uses built-in DoH with Direct outbound dialing, and `status` shows `resolver: doh`.
...
7. `v0.6.0`: real Steam smoke tests and rule coverage.
